...
- [ ] The app does **not** import `@napplet/shim` — the runtime injects
  `window.napplet` before napplet code runs.
...
- [ ] The app feature-detects optional NAPs with `window.napplet?.<domain>` and
  guards the call itself — a domain can be injected without every helper.
