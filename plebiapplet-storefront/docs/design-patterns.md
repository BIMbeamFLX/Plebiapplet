...
The runtime injects `window.napplet` before napplet code runs. Napplets do not
import `@napplet/shim` — that package is for runtimes that need the first-party
installer. App code imports typed helpers only.

```ts
import { outbox, storage, identity } from '@napplet/sdk';
```

SDK methods read `window.napplet` at call time, so app modules can import
helpers freely.

## Feature Detection

Current packages expose no `window.napplet.shell`, `shell.ready()`, or
`shell.supports()`. Available domains are present as properties on the injected
namespace; unavailable ones are absent.
...
Presence checks are advisory in a second sense too: a domain can be injected
without implementing every helper, and a user-triggered call can still fail
because the shell denied a grant, hit a quota, or rejected a policy input.
Guard the call as well as the domain, and always surface operation errors.
